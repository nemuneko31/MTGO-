/* ===== v7.9.48 Client Bundle Runtime Bootstrap ===== */
(function(){
  "use strict";
  const VERSION="7.9.48";
  const PROTOCOL="cpt-v7.9.48-bundle-runtime-status";
  const startedAt=new Date().toISOString();
  const runtime=globalThis.CPT_V7948_CLIENT_BUNDLE={version:VERSION,protocol:PROTOCOL,loaded:true,complete:false,error:null,startedAt,completedAt:null};
  function host(){return document.body||document.documentElement;}
  function install(){
    try{
      if(!host())return;
      document.body.dataset.v7948="bundle-loading";
      document.title="カードゲーム練習卓 v7.9.48 統合bundle読込中";
      let b=document.getElementById("v7948BundleStatus");
      if(!b){
        b=document.createElement("button");
        b.type="button";
        b.id="v7948BundleStatus";
        b.setAttribute("aria-live","polite");
        b.style.cssText="position:fixed;top:8px;right:8px;z-index:2147483647;border:1px solid #b4862e;border-radius:999px;padding:5px 10px;background:#332b18;color:#ffe5a4;font:600 12px system-ui,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.35);cursor:pointer";
        b.addEventListener("click",function(){
          const s=globalThis.CPT_V7948_CLIENT_BUNDLE||{};
          const message=s.complete?"統合bundle v7.9.48 は正常に初期化されています。":("統合bundleの初期化が完了していません。"+(s.error?"\n\n"+s.error:""));
          try{if(typeof openModal==="function")openModal("統合bundle状態",`<div class=\"note\">${String(message).replace(/[&<>]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[m])).replace(/\\n/g,"<br>")}</div>`,`<button class=\"btn\" data-app=\"closem\">閉じる</button>`);else alert(message);}catch(_){alert(message);}
        });
        host().appendChild(b);
      }
      b.textContent="統合bundle v7.9.48 読込中";
      b.title="bundleファイルは配信済み。全拡張の初期化を確認しています";
    }catch(_){ }
  }
  function recordError(e){
    try{
      const file=String(e&&e.filename||"");
      const stack=String(e&&e.error&&e.error.stack||e&&e.message||"");
      if(file.includes("v7948-client-bundle")||stack.includes("v7948-client-bundle"))runtime.error=String(e&&e.message||e&&e.error&&e.error.message||"bundle初期化エラー");
    }catch(_){ }
  }
  window.addEventListener("error",recordError,true);
  install();
  [50,200,800].forEach(ms=>setTimeout(install,ms));
  setTimeout(function(){
    if(runtime.complete)return;
    install();
    const b=document.getElementById("v7948BundleStatus");
    if(b){
      b.textContent="統合bundle v7.9.48 初期化未完了";
      b.style.borderColor="#b34d4d";b.style.background="#3a2020";b.style.color="#ffd1d1";
      b.title=runtime.error||"bundle内の拡張初期化が途中で停止した可能性があります。クリックして状態を確認してください";
    }
    try{document.body.dataset.v7948="bundle-incomplete";}catch(_){ }
  },3000);
})();
...
/* ===== v7.9.48 Client Bundle Runtime Completion ===== */
(function(){
  "use strict";
  const s=globalThis.CPT_V7948_CLIENT_BUNDLE||{};
  s.complete=true;s.error=null;s.completedAt=new Date().toISOString();
  try{
    document.body.dataset.v7948="bundle-complete";
    document.title="カードゲーム練習卓 v7.9.48 統合bundle版";
    const b=document.getElementById("v7948BundleStatus");
    if(b){
      b.textContent="統合bundle v7.9.48 有効";
      b.style.borderColor="#4d9a68";b.style.background="#173322";b.style.color="#c9f6d5";
      b.title="v7.9.30～v7.9.46のクライアント拡張を1本のbundleから初期化済み";
    }
  }catch(_){ }
})();
